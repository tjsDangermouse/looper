...
describe('road and path instructions',()=>{
 it('calls a straight pavement-road-pavement sequence one unnamed crossing',()=>{
  const route:Route={...sample,geometry:{type:'LineString',coordinates:[[0,0],[0,.001],[.00001,.00114],[.00001,.002]]},steps:[
   {instruction:'Head along path',distanceMeters:111,durationSeconds:80,roadClass:'footway',startIndex:0,endIndex:1},
   {instruction:'Bear right onto Main Road',distanceMeters:16,durationSeconds:12,maneuver:'keep-right',road:'Main Road',roadClass:'primary',startIndex:1,endIndex:2},
   {instruction:'Turn right',distanceMeters:96,durationSeconds:69,maneuver:'turn-right',roadClass:'footway',startIndex:2,endIndex:3},
   {instruction:'Arrive',distanceMeters:0,durationSeconds:0,maneuver:'finish',startIndex:3,endIndex:3},
  ]}
  const checked=normaliseWalkingSteps(route)
  expect(checked.steps.map(step=>step.instruction)).toEqual(['Head along path','Cross the road and continue straight','Arrive'])
  expect(checked.steps[1].instruction).not.toContain('Main Road')
  expect(checked.steps[1].distanceMeters).toBe(112)
  expect(turnKind(checked.steps[1])).toBe('straight')
  const backwards=reverseRoute(route)
  expect(backwards.steps.map(step=>step.instruction)).toEqual(['Set off along the loop','Cross the road and continue straight','Arrive at your starting point'])
 })
